  ...
  Messager.prototype = {
    ...
    unbind: function(listener, sender, type, callback) {
      ...
      for (var i=this.binds.length; i>0;) {
        bind = this.binds[--i];
        if (lid && lid != bind[msg.listener]) {continue;}
        if (sid && sid != bind[msg.sender]) {continue;}
        if (type && type != bind[msg.type]) {continue;}
        if (callback && callback != bind[msg.func]) {continue;}
