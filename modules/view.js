define(function(require,exports) {
	 ...
	 */
	function Container(config,parent,idObject){
		this.config = $.extend({
			// 标签名
			"tag":"div"
			// 标签添加容器或目标容器的jq选择器
		...
		,{
			...
			,show:function(config){
				...
				var me = this
					,callBack = function(){
						me.afterShow();
						if(me.config.autoHide){
							var el = [me.el];
							if(me.config.autoHide.andSelf){
								el = el.concat(me.config.autoHide.andSelf);
							}
							me.auotHideHandler = _hideOnDocumentClick(
								el
								,me.hide
								,me
							);
			...
			,hide:function(config){
				...
				var me = this
					,callBack = function(){
						me.afterHide();
						if(me.config.autoHide){
							me.auotHideHandler = null;
						}
						me.cast('containerHide');
					}
			...
			,destroy:function(){
				if (this.doms){
					for (var i in this.doms){
						if (this.doms.hasOwnProperty(i) && this.doms[i] && this.doms[i].jquery){
							this.doms[i].remove();
						}
					}
				}
	 ...
	 * @param {Object} idObject 当前实例的相关信息<系统自动追加>
	 */
	function Layout(config,parent,idObject){
		if(!config.target){
			return false;
		}
		this.config = $.extend(
